...
const PITFALLS = [
  ...
  },
  {
    ...
    body: '这类站不同模型族走的接口不一样：Claude 走 /v1/messages，GPT 走 /v1/responses，其余走 /v1/chat/completions。用错了同样返回「当前 API 不支持所选模型」—— 看着像模型不存在，其实是端点不对。上面的自检会按模型名自动挑端点，跟着它给的地址改就行。',
  },
  {
    title: 'key 里混进空格换行',
